...
public class BCUITweenerEditor : Editor
{
	...
	protected void DrawCommonProperties ()
	{
		...
		if (BCEditorTools.DrawHeader("Tweener"))
		{
            ...
            bool reverseWhenClose = false;
            if (style== BCUITweener.Style.Once)
            {
                GUILayout.BeginHorizontal();
                reverseWhenClose = EditorGUILayout.Toggle("界面关闭时倒播", tw.playReverseWhenClose);
                GUI.color = Color.yellow;
                EditorGUILayout.LabelField("<-目前仅执行TweenScale倒播！->");
			...
			if (GUI.changed)
			{
                ...
                tw.playReverseWhenClose = reverseWhenClose;
                //tw.tweenGroup = tg;
                tw.duration = dur;
				tw.delay = del;

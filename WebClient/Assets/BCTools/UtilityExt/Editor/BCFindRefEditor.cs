...
public class BCFindRefEditor : EditorWindow
{
	...
	void OnGUI()
	{
        ...
        searchScriptRef = EditorGUILayout.Toggle("FindScript:", searchScriptRef);
        EditorGUILayout.EndHorizontal();
	...
	void DoSearch()
	{
        if (selectFile != null)
        {
            if (searchScriptRef)
            {
                selectScript = selectFile.GetType();
                if(selectScript == null)
                {
                    EditorUtility.DisplayDialog("Warning", "Choose a monoscript first!", "OK");
                    return;
                }
            }
            else
    ...
    void AddScriptFileResult(DirectoryInfo dinfo)
    {
        if (dinfo.Exists)
        {
            FileInfo[] finfo = null;
            ...
            for (int i = 0; finfo != null && i < finfo.Length; i++)
            {
                ...
                GameObject go = AssetDatabase.LoadAssetAtPath(tempstr, typeof(Object)) as GameObject;
                if (go != null && go.GetComponentsInChildren(selectScript,true).Length > 0)
                {
            ...
            {
                AddScriptFileResult(dinfoarray[j]);
            }
        }
    }
